...
Q <- quantile(train_camera$units, probs=c(.25, .75), na.rm = FALSE)
iqr <- IQR(train_camera$units)
up <-  Q[2]+1.5*iqr # Upper Range  
low<- Q[1]-1.5*iqr # Lower Range???
train_camera <- subset(train_camera, units > (Q[1] - 1.5*iqr) & units < (Q[2]+1.5*iqr))
hist(train_camera$units)
boxplot(train_camera$units)
boxplot(train_camera$gmv)
hist(train_camera$units)
...
fit_camera_3 = lm(gmv ~ units  + product_mrp + 
...
fit_camera_4 = lm(gmv ~ units  + product_mrp + 
                     Content_Marketing_Investment +  Online_Marketing_Investment + 
                    Discount, data = train_camera)
...
train_camera$yhat = predict(fit_camera_4)
...
df_c1 = df_c1 %>% 
  group_by(Month) %>%
  summarise(total_gmv = mean(gmv), total_yhat = mean(yhat))


df_c1 <- melt(df_c1 ,  id.vars = 'Month', variable.name = 'series')

ggplot(df_c1, aes(Month, value)) +
  geom_line(aes(colour = series),size=2) + theme_classic(base_size = 16)
...
Q <- quantile(test_camera$units, probs=c(.25, .75), na.rm = FALSE)
iqr <- IQR(test_camera$units)
up <-  Q[2]+1.5*iqr # Upper Range  
low<- Q[1]-1.5*iqr # Lower Range???
test_camera <- subset(test_camera, gmv > (Q[1] - 1.5*iqr) & gmv < (Q[2]+1.5*iqr))
hist(test_camera$gmv)
...
res = rstandard(fit_camera_4)   # standardized residual
yhat = predict(fit_camera_4) # predicted y
...
# X-outliers 
barplot(hatvalues(fit_camera_4)) 
which(hatvalues(fit_camera_4)>0.02)
...
barplot(cooks.distance(fit_camera_4))
which(cooks.distance(fit_camera_4)>0.03)
...
fit_speaker_6 = lm(gmv ~ units  + sla + product_mrp + Discount + product_procurement_sla + 
                     Radio_Investment + Sponsorship_Investment + Premium_Product + Holiday   + 
                     SEM_Investment, data = train_speaker)
...
# X-outliers 
barplot(hatvalues(fit_speaker_6)) 
which(hatvalues(fit_speaker_6)>0.3)
...
# Influential observations
barplot(cooks.distance(fit_speaker_6))
which(cooks.distance(fit_speaker_6)>0.6)
...
fit_homeaudio_4 = lm(gmv ~ units + gmv_per_unit + sla + 
                       product_procurement_sla + Sponsorship_Investment + 
                       SEM_Investment + product_mrp + order_payment_type + TV_Investment  + 
                       Premium_Product, data = train_homeaudio)

summary(fit_homeaudio_4)
...
res = rstandard(fit_homeaudio_4)   # standardized residual
yhat = predict(fit_homeaudio_4) # predicted y
...
# X-outliers 
barplot(hatvalues(fit_homeaudio_4)) 
which(hatvalues(fit_homeaudio_4)>0.3)
...
# Influential observations
barplot(cooks.distance(fit_homeaudio_4))
which(cooks.distance(fit_homeaudio_4)>1.4)
...
# X-outliers 
barplot(hatvalues(fit_game_3)) 
which(hatvalues(fit_game_3)>0.2)
...
# Influential observations
barplot(cooks.distance(fit_game_3))
which(cooks.distance(fit_game_3)>0.2)
...
# X-outliers 
barplot(hatvalues(fit_cam_acc_3)) 
which(hatvalues(fit_cam_acc_3)>0.2)
...
# Influential observations
barplot(cooks.distance(fit_cam_acc_3))
which(cooks.distance(fit_cam_acc_3)>0.2)
